...
def convert(src_file_path, dest_file_path):
    ...
    with open(src_file_path) as f:
        ...
        for line_indx, line in enumerate(lines):
            ...
            if ((line_indx - 1) >= num_nodes and (len(line_elements) == 1)):
                ...
                node_degree[int(line_elements[0])] = node_degree.get(int(line_elements[0]), 0) + 1
                node_degree[int(line_elements[1])] = node_degree.get(int(line_elements[1]), 0) + 1
    ...
    with open(dest_file_path, 'w') as f:
        ...
        for node_id, node_label in nodes_dict.items():
            f.write(f"v {node_id} {node_label} {node_degree[node_id]}\n")
            
        for src in edges_dict.keys():
            for dest in edges_dict[src]:
                f.write(f"e {src} {dest}\n")
