...
TIMEOUT_MAX = 3600 # 1 hr


def run_egsm(exe_path: str, res_dir: str, log_file: str, error_file: str, query_path: str, target_path: str, results_dict: dict, args: list, query_size: str, query_indx: str, labels: str):
    print("Running EGSM")
    command = [exe_path, f"-q{query_path}", f"-d{target_path}"] #+ args
    # print(command)
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            for indx, row in enumerate(rows):
                # print(row)
                if 'Matching time: , time (ms):' in row:
                    # example: match used: 303 ms
                    running_time = float(row.split('Matching time: , time (ms): ')[-1].split('(host)')[0]) / 1000 # from ms to s
                    print(f"Running time: {running_time}")
                    
                if '# Matches:' in row:
                    # example:  # Matches: 55
                    num_sol = int(row.split('# Matches:')[-1].split('\n')[0])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--database_foder', type=str,
                        default='/dataset/DBLP/EGSM_format')
    parser.add_argument('--bin_path', type=str, default='/graph-matching-analysis/baseline_algorithms/EGSM/build/EGSM')
    ...
    # create log file 
    log_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), f"log_{args.dataset_name}_query_size_{args.query_size}.txt")
    
    error_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), f"log_{args.dataset_name}_query_size_{args.query_size}_error.txt")
    ...
    for label_indx, label_size in enumerate(LABEL_SIZE):
        ...
        query_files = glob.glob(f"{final_query_folder}/*.sub.grf") if args.dataset_name == 'DBLP' else glob.glob(f"{final_query_folder}/*")
        ...
        if args.dataset_name == 'DBLP':
            ...
            data_path = f"{args.database_foder}/data.graph"
        ...
        for idx, query_file in enumerate(query_files):
            ...
            if qs in results_dict.keys() and qi in results_dict[qs].keys() and lab_size in results_dict[qs][qi].keys():
                ...
                print(log_file)
                results_dict = run_egsm(exe_path=args.bin_path,
